...
const imageRoles = new Set(["product-image", "design-reference"]);
const attachmentRoles = new Set([...imageRoles, "supporting-material"]);
const defaultTypes = {
  png: "image/png",
  jpg: "image/jpeg",
  jpeg: "image/jpeg",
...
export function readImageAttachmentSources(rawValue, attachments) {
  ...
  for (const attachment of (attachments ?? []).filter((item) => item.kind === "image")) {
    available.set(attachmentKey(attachment.name, attachment.role), attachment);
    if (!available.has(attachment.name)) available.set(attachment.name, attachment);
  }
  const sources = [];
  for (const item of rawValue) {
    const input = readObject(item);
    const name = safeFileName(readString(input.name));
    const role = attachmentRole(readString(input.role), "image");
    const attachment = name ? available.get(attachmentKey(name, role)) ?? available.get(name) : undefined;
...
export function materialLines(product) {
  ...
  const groups = [
    ["product-image", "상품 이미지(원본 유지)"],
    ["design-reference", "디자인 레퍼런스(스타일 참고)"],
    ["supporting-material", "자료 파일"],
  ];
  for (const [role, label] of groups) {
    const groupAttachments = attachments.filter((attachment) => attachment.role === role);
    for (const attachment of groupAttachments) lines.push(`- ${label}: ${attachmentSummary(attachment)}`);
  }
  return lines;
}
...
function readAttachment(value) {
  ...
  const role = attachmentRole(readString(input.role), kind);
  if (imageRoles.has(role) && kind !== "image") {
    return { ok: false, error: `${role} attachments must be image files: ${name}` };
  }
  if (role === "supporting-material" && kind === "image") {
    return { ok: false, error: `supporting-material attachments must be document files: ${name}` };
  }
...
function attachmentSummary(attachment) {
  const parts = [attachment.name, `role=${attachment.role}`, roleLabel(attachment.role), attachment.type, `${attachment.size} bytes`, attachment.kind];
  if (attachment.preview) parts.push("preview available");
  if (attachment.textPreview) parts.push(`텍스트 요약: ${attachment.textPreview}`);
  return parts.join(" | ");
}

function attachmentRole(value, kind) {
  if (value && attachmentRoles.has(value)) return value;
  return kind === "image" ? "product-image" : "supporting-material";
}
...
function attachmentKey(name, role) {
  return `${role}:${name}`;
}
