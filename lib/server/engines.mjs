...
export async function runEngine(input, prompt, options = {}) {
  return input.engine.mode === "byok-http"
    ? runByokProvider(input, prompt, options)
    : runLocalCli(input, prompt, options);
}

async function runLocalCli(input, prompt, options = {}) {
  ...
  try {
    return await spawnInvocation({ invocation, prompt, engineId, label, stage, timeoutMs, logs, signal: options.signal });
  } finally {
    await invocation.cleanup?.();
  }
}

function spawnInvocation({ invocation, prompt, engineId, label, stage, timeoutMs, logs, signal }) {
  if (signal?.aborted) return Promise.resolve(cancelledRunResult(logs, "local CLI cancelled", `${label} ${stage} 단계 실행이 시작 전에 취소되었습니다.`));
  return new Promise((resolveRun) => {
    ...
    child.on("error", (error) => {
      ...
      cleanupTimers();
      if (aborted) {
        resolveRun(cancelledRunResult(logs, "local CLI cancelled", `${label} ${stage} 단계 실행을 취소했습니다.`, stdout));
        return;
      }
    ...
    child.on("close", async (code, signal) => {
      ...
      if (aborted) {
        resolveRun(cancelledRunResult(logs, "local CLI cancelled", `${label} ${stage} 단계 실행을 취소했습니다.`, output));
        return;
      }
      if (timedOut) {
...
function cancelledRunResult(logs, title, message, output = "") {
  return {
    ok: false,
    aborted: true,
    output,
    error: message,
    logs: [...logs, logEntry("warning", title, message)],
  };
}
