...
import { editGeneratedImage } from "./image-edit.mjs";
...
async function routeRequest(request, response, localToken, generationJobs) {
  ...
  if (request.method === "POST" && url.pathname === "/api/generate-jobs") {
    if (!allowApiWrite(request, response, localToken)) return;
    const job = await generationJobs.start(await readJsonBody(request), { ephemeral: isEphemeralJobRequest(request) });
    sendJson(response, 202, { ok: true, job });
    return;
  }
  if (request.method === "POST" && url.pathname === "/api/images/edit") {
    if (!allowApiWrite(request, response, localToken)) return;
    const result = await editGeneratedImage(await readJsonBody(request));
  ...
  if (request.method === "GET" && url.pathname === "/api/generate-jobs") {
    ...
    sendJson(response, 200, { ok: true, jobs: await generationJobs.list({ includeEphemeral: isEphemeralJobRequest(request) }) });
    return;
  }
  const jobPath = parseJobPath(url.pathname);
  if (request.method === "GET" && jobPath && !jobPath.cancel && !jobPath.delete) {
    if (!allowApiRead(request, response, localToken)) return;
  ...
  if (request.method === "POST" && jobPath?.delete) {
    if (!allowApiWrite(request, response, localToken)) return;
    const result = await generationJobs.delete(jobPath.id);
    if (result.status === "deleted") {
      sendJson(response, 200, { ok: true, jobs: await generationJobs.list({ includeEphemeral: isEphemeralJobRequest(request) }) });
      return;
    }
    if (result.status === "active") {
      sendJson(response, 409, { ok: false, error: { code: "JOB_ACTIVE", message: "완료, 실패, 취소된 작업만 삭제할 수 있습니다." }, job: result.job });
      return;
    }
    sendJson(response, 404, { ok: false, error: { code: "NOT_FOUND", message: "Generation job not found" } });
    return;
  }
...
function isEphemeralJobRequest(request) {
  return request.headers["x-store-maker-ephemeral-job"] === "1";
}
...
function parseJobPath(pathname) {
  const match = pathname.match(/^\/api\/generate-jobs\/([^/]+)(?:\/(cancel|delete))?$/u);
  if (!match) return undefined;
  try {
    return {
      id: decodeURIComponent(match[1]),
      cancel: match[2] === "cancel",
      delete: match[2] === "delete",
    };
  } catch (error) {
    return undefined;
  }
