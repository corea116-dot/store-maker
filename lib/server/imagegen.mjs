...
export async function runImageGeneration(input, options = {}) {
  ...
  let references;
  try {
    const uploadedReferences = config.useReference
      ? await persistReferenceImages(input.runtime?.imageAttachmentSources ?? [], uploadDir, uploadRelativeDir)
      : [];
    const runtimeReferences = config.useReference ? runtimeReferenceImages(input.runtime?.imageReferenceFiles) : [];
    references = [...uploadedReferences, ...runtimeReferences];
  } catch (error) {
  ...
  try {
    ...
    const importLogs = collection.importedFromCodexHome
      ...
      : [];
    const moodCounts = normalizedMoodCounts(config);
    const moodMode = normalizedMoodMode(config);
    return {
      ok: true,
      images: {
        ...
        moodMode,
        sameMoodCount: moodCounts.same,
        variedMoodCount: moodCounts.varied,
        ratio: config.ratio,
        style: config.style,
        imageBriefs,
        background: effectiveBackground(config),
        referenceFiles: references.map(({ name, role, type, size }) => ({ name, role, type, size })),
        quality,
        files,
        images: files,
...
function composeImagePrompt(input, { outputDir, outputRelativeDir, references, imageCount, imageBriefs }) {
  ...
  const attachmentSummary = input.product.attachments?.length
    ? input.product.attachments.map(attachmentPromptLine).join("\n")
    : "- 제공 없음";
  const referenceSummary = references.length
    ? references.map(referencePromptLine).join("\n")
  ...
  const moodLines = imageMoodPromptLines(config);
  const editLines = imageEditPromptLines(config);
  const imageUseRules = imageReferenceRuleLines(references);
  ...
  if (isAdGenerationMode(input)) {
    ...
    return [
      ...
      ...requiredInclusionLines(input.product),
...
function attachmentPromptLine(attachment) {
  return `- ${attachment.name} | role=${attachment.role ?? "supporting-material"} (${roleLabel(attachment.role)}) | ${attachment.type} | ${attachment.size} bytes | ${attachment.kind}`;
}

function referencePromptLine(reference) {
  return `- ${reference.name} | role=${reference.role ?? "product-image"} (${roleLabel(reference.role)}) | ${reference.type}, ${reference.size} bytes | attached via --image | ${reference.relativePath} | ${reference.absolutePath}`;
}

function imageReferenceRuleLines(references) {
  const productImageCount = references.filter((reference) => reference.role === "product-image" || !reference.role).length;
  const designReferenceCount = references.filter((reference) => reference.role === "design-reference").length;
  return [
    `- 상품 이미지(product-image) ${productImageCount}개는 실제 상품 기준입니다. 형태, 색상, 로고, 재질, 구성품, 고유 특징을 임의로 바꾸지 말고 첨부 이미지 그대로 유지하세요.`,
    `- 디자인 레퍼런스(design-reference) ${designReferenceCount}개는 분위기, 구도, 조명, 배경, 레이아웃 참고용입니다. 레퍼런스 속 상품, 브랜드, 로고를 복사하거나 상품 이미지의 특징을 대체하지 마세요.`,
    "- 상품 이미지와 디자인 레퍼런스가 모두 있으면 상품 이미지를 우선하고, 디자인 레퍼런스는 연출 방향에만 반영하세요.",
  ];
}
...
function imageReferenceRole(role) {
  return role === "design-reference" ? "design-reference" : "product-image";
}
...
function composeImageRetryPrompt(input, { outputDir, outputRelativeDir, references, imageCount, generatedCount, files, imageBriefs }) {
  ...
  return [
    ...
    "",
    "## 이미지 사용 규칙",
    ...imageReferenceRuleLines(references),
    "",
    ...
    `상품명: ${input.product.name}`,
    `요구사항: ${input.product.requirements}`,
    ...requiredInclusionLines(input.product).map((line) => line.replace(/^- /u, "")),
  ].join("\n");
}
...
function buildImageBriefs(input, imageCount) {
  ...
  const { same } = normalizedMoodCounts(input.imageGeneration);
  const moodMode = normalizedMoodMode(input.imageGeneration);
  const adBriefs = adImageBriefs(input);
  const background = effectiveBackground(input.imageGeneration);
  return files.map((filename, index) => {
    const moodGroup = index < same ? "same" : "varied";
    const variedIndex = Math.max(0, index - same);
    const definition = moodGroup === "varied"
      ? imageStyleDefinitions[variedIndex % imageStyleDefinitions.length]
    ...
    const purpose = moodGroup === "varied"
      ? `다른 무드 · ${definition.purpose}${cycle > 1 ? ` 변주 ${cycle}` : ""}`
    ...
    const visualPrompt = [
      ...
      `무드 그룹: ${moodGroup === "same" ? "동일한 무드" : "다른 무드"}`,
      `목적: ${purpose}`,
      `상품: ${input.product.name}`,
      ...(input.product.requiredInclusions ? [`필수 포함: ${input.product.requiredInclusions}`] : []),
...
function normalizedMoodMode(config) {
  const same = Number.isSafeInteger(config?.sameMoodCount) ? config.sameMoodCount : undefined;
  const varied = Number.isSafeInteger(config?.variedMoodCount) ? config.variedMoodCount : undefined;
  if (config?.moodMode === "mixed" || config?.moodMode === "varied" || config?.moodMode === "consistent") return config.moodMode;
  if (same > 0 && varied > 0) return "mixed";
  if (varied > 0 || DIVERSITY_STYLE_OPTIONS.has(config?.style)) return "varied";
  return "consistent";
}

function normalizedMoodCounts(config) {
  const total = normalizedExpectedCount(config?.imageCount ?? config?.count);
  const same = Number.isSafeInteger(config?.sameMoodCount) ? config.sameMoodCount : undefined;
  const varied = Number.isSafeInteger(config?.variedMoodCount) ? config.variedMoodCount : undefined;
  if (same !== undefined && varied !== undefined && same + varied >= 1 && same + varied <= maxImageCount) return { same, varied };
  const mode = normalizedMoodMode(config);
  if (mode === "varied") return { same: 0, varied: total };
  if (mode === "mixed") {
    const mixedSame = Math.ceil(total / 2);
    return { same: mixedSame, varied: total - mixedSame };
  }
  return { same: total, varied: 0 };
}

function imageMoodPromptLines(config) {
  const counts = normalizedMoodCounts(config);
  const mode = normalizedMoodMode(config);
  const label = mode === "varied"
    ? "각기 다른 무드"
    : mode === "mixed"
      ? "동일+다른 무드 혼합"
      : "일관된 무드";
  return [
    `- 무드 구성: ${label}`,
    `- 동일한 무드 결과: ${counts.same}개`,
    `- 다른 무드 결과: ${counts.varied}개`,
  ];
}

function imageEditPromptLines(config) {
  if (!config.editInstruction) return [];
  return [
    "",
    "## 개별 이미지 추가 수정",
    `- 수정 대상: ${config.sourceImageName ?? "선택된 생성 이미지"}`,
    `- 수정 요청: ${config.editInstruction}`,
    "- 첨부된 reference 이미지를 기준으로 형태와 핵심 구도는 유지하고, 수정 요청에 해당하는 부분만 반영하세요.",
    "- 새 수정본 1장을 별도 이미지 파일로 저장하세요. 기존 원본 파일은 덮어쓰지 마세요.",
  ];
}
...
function adImageBriefs(input) {
  if (!isAdGenerationMode(input)) {
    return [
      ...
      ...(input.product.requiredInclusions ? [`${input.product.name} 필수 포함 정보/문구. ${input.product.requiredInclusions}`] : []),
...
function requiredInclusionLines(product) {
  return product.requiredInclusions ? [`- 필수 포함 정보/문구: ${product.requiredInclusions}`] : [];
}
...
async function persistReferenceImages(sources, uploadDir, uploadRelativeDir) {
  ...
  for (let index = 0; index < sources.length; index += 1) {
    ...
    references.push({
      name: source.name,
      role: imageReferenceRole(source.role),
      type: source.type,
      size: buffer.length,
...
function runtimeReferenceImages(files) {
  if (!Array.isArray(files)) return [];
  return files.flatMap((file) => {
    if (!file || typeof file !== "object") return [];
    if (typeof file.absolutePath !== "string" || !file.absolutePath.trim()) return [];
    return [{
      name: typeof file.name === "string" && file.name.trim() ? file.name.trim() : basename(file.absolutePath),
      role: imageReferenceRole(file.role),
      type: typeof file.type === "string" && file.type.trim() ? file.type.trim() : mimeType(extensionOf(file.absolutePath)),
      size: typeof file.size === "number" && Number.isFinite(file.size) ? Math.max(0, Math.round(file.size)) : 0,
      relativePath: typeof file.relativePath === "string" && file.relativePath.trim() ? file.relativePath.trim() : basename(file.absolutePath),
...
function fallbackManifest({ files, runId, outputRelativeDir, input, references, providerLabel, imageBriefs = [] }) {
  return {
    ...
    promptSummary: {
      ...
      referenceCount: references.length,
      references: references.map(({ name, role, type, size }) => ({ name, role, type, size })),
    },
...
function ensureCodexExecArgs(args) {
  const { args: withoutApproval } = extractOptionPair(args, "--ask-for-approval", "-a");
  const execIndex = withoutApproval.indexOf("exec");
  if (execIndex >= 0) {
    return [...withoutApproval.slice(0, execIndex), "exec", ...withoutApproval.slice(execIndex + 1)];
  }
  return ["exec", ...withoutApproval];
}
