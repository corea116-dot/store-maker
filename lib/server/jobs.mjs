...
export function createGenerationJobManager({ run }) {
  ...
  return {
    ...
    async list() {
      await ready;
      return sortedHistory().map((job) => publicJob(job, { includeResult: false }));
    },
  ...
  function persistHistory() {
    const snapshot = JSON.stringify({
      version: 1,
      jobs: history.map((job) => storedJob(job)),
    }, null, 2);
  ...
  function trimHistory() {
    ...
    const terminal = sortJobsByNewestCreated(history.filter((job) => TERMINAL_STATUSES.has(job.status))).slice(0, JOB_HISTORY_LIMIT);
    history = sortJobsByNewestCreated([...active, ...terminal]);
  }
