...
export function createGenerationJobManager({ run }) {
  ...
  return {
    ...
    async list() {
      await ready;
      return history.map((job) => publicJob(job, { includeResult: false }));
    },
  ...
  async function loadHistory() {
    try {
      ...
      history = loadedJobs.filter(Boolean).slice(0, JOB_HISTORY_LIMIT);
  ...
  function trimHistory() {
    ...
    const terminal = history.filter((job) => TERMINAL_STATUSES.has(job.status)).slice(0, JOB_HISTORY_LIMIT);
