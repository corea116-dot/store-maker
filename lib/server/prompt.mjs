...
import { defaultImageStyle, diversityImageStyles, imageMoodModes, imageStyleOptions, maxImageCount, minImageCount } from "../../assets/image-options.js";
...
export function parseGenerationRequest(value) {
  ...
  const product = {
    ...
    requiredInclusions: readString(productInput.requiredInclusions) ?? readString(productInput.mustInclude) ?? readString(productInput.mustIncludeText),
...
function fallbackMarkdown(input) {
  return [
    ...
    "",
    `요구사항: ${input.product.requirements}`,
    ...(input.product.requiredInclusions ? ["", `필수 포함 정보/문구: ${input.product.requiredInclusions}`] : []),
  ].join("\n");
}
...
function imageSectionHtml(input, images) {
  ...
  const files = images.files.map((file) => {
    const metadata = generatedImageAttributes(file, images);
    const alt = `${input.product.name} ${file.filename}`;
    return `
    <figure class="generated-image-card${file.isPlaceholder ? " generated-image-card-placeholder" : ""}" data-generated-image-card>
      <div class="generated-image-frame">
        <button class="generated-image-open" type="button" data-action="open-generated-image" ${metadata} aria-label="${escapeAttribute(`${alt} 큰 화면으로 보기`)}">
          <img src="${escapeAttribute(file.url)}" alt="${escapeAttribute(alt)}" />
      ...
      <div class="generated-image-actions">
        <button class="btn" type="button" data-action="open-generated-image" ${metadata}>큰 화면</button>
        <a class="btn" href="${escapeAttribute(file.url)}" target="_blank" rel="noopener">원본 열기</a>
      </div>
    </figure>
  `;
  }).join("");
...
function generatedImageAttributes(file, images) {
  const values = {
    "data-image-url": file.url,
    "data-image-filename": file.filename,
    "data-image-relative-path": file.relativePath,
    "data-image-style": file.style ?? images.style,
    "data-image-purpose": file.purpose ?? file.brief?.purpose,
    "data-image-type": file.mimeType ?? file.type,
  };
...
function imageMoodMarkdown(images) {
  if (!Number.isSafeInteger(images.sameMoodCount) || !Number.isSafeInteger(images.variedMoodCount)) return [];
  return [`- 무드 구성: 동일한 무드 ${images.sameMoodCount}개 / 다른 무드 ${images.variedMoodCount}개`];
}

function imageMoodHtml(images) {
  if (!Number.isSafeInteger(images.sameMoodCount) || !Number.isSafeInteger(images.variedMoodCount)) return "";
  return `<p class="generated-image-mood-count">${escapeHtml(`동일한 무드 ${images.sameMoodCount}개 / 다른 무드 ${images.variedMoodCount}개`)}</p>`;
}
...
function readImageGeneration(value) {
  ...
  const style = readChoice(input.style, imageStyleOptions, defaultImageStyle);
  const moodPlan = readMoodPlan(input, imageCount.value, style);
  if (!moodPlan.ok) return moodPlan;
  return { ok: true, value: {
    ...
    imageCount: moodPlan.value.imageCount,
    count: moodPlan.value.imageCount,
    moodMode: moodPlan.value.moodMode,
    sameMoodCount: moodPlan.value.sameMoodCount,
    variedMoodCount: moodPlan.value.variedMoodCount,
...
function readMoodPlan(input, legacyImageCount, style) {
  const hasMoodCounts = Object.hasOwn(input, "sameMoodCount") || Object.hasOwn(input, "variedMoodCount");
  if (!hasMoodCounts) {
    const moodMode = diversityImageStyles.includes(style) ? "varied" : "consistent";
    return {
      ok: true,
      value: {
        imageCount: legacyImageCount,
        moodMode,
        sameMoodCount: moodMode === "varied" ? 0 : legacyImageCount,
        variedMoodCount: moodMode === "varied" ? legacyImageCount : 0,
      },
    };
  }
  const same = readZeroOrPositiveInt(input.sameMoodCount);
  if (!same.ok) return { ok: false, error: "imageGeneration.sameMoodCount must be an integer from 0-20" };
  const varied = readZeroOrPositiveInt(input.variedMoodCount);
  if (!varied.ok) return { ok: false, error: "imageGeneration.variedMoodCount must be an integer from 0-20" };
  const imageCount = same.value + varied.value;
  if (imageCount < minImageCount || imageCount > maxImageCount) {
    return { ok: false, error: `sameMoodCount + variedMoodCount must be ${minImageCount}-${maxImageCount}` };
  }
  return {
    ok: true,
    value: {
      imageCount,
      moodMode: readMoodMode(input.moodMode, same.value, varied.value),
      sameMoodCount: same.value,
      variedMoodCount: varied.value,
    },
  };
}

function readZeroOrPositiveInt(value) {
  if (typeof value !== "number" || !Number.isSafeInteger(value) || value < 0 || value > maxImageCount) return { ok: false };
  return { ok: true, value };
}

function readMoodMode(value, sameMoodCount, variedMoodCount) {
  if (imageMoodModes.includes(value)) return value;
  if (sameMoodCount > 0 && variedMoodCount > 0) return "mixed";
  if (variedMoodCount > 0) return "varied";
  return "consistent";
}
